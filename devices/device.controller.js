...
const emailTemplate = fs.readFileSync(
  "view/mail_templates/device_created.html",
  "utf-8"
);
...
router.get("/reports/:fieldName", reports);
...
function sendEmailWithAttachment(excelBuffer) {
  ...
  const mailOptions = {
    ...
    to: [
      "sharathkumarpatil06@gmail.com",
      "support@sqcpack.co.in",
      "nandapqsystems@gmail.com",
      "product@pqsi.co.in",
    ],
  ...
  try {
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (paramValue) {
        ...
        if (
          paramValue &&
          (paramValue < lowerLimit || paramValue > upperLimit)
        ) {
...
//     if (!fieldName) {
//       return res.json({ message: 'Required FieldName' });
//     }

//     // Assuming getAllDevicesByField returns an array of devices based on the specified field
//     // Modify the query to order by createdAt in descending order and limit the results to 10
//     const devices = await deviceService.getTop10DevicesByField(fieldName);
...
//         {
//           name: fieldName,
//           data: devices.map((device) => device[fieldName]),
//         },
//       ],
//       options: {
//         chart: {
...
//         },
//         xaxis: {
//           categories: devices.map((device) => device.createdAt),
//         },
//         yaxis: [
//           {
//             title: {
//               text: `${fieldName} Values`,
//             },
//           },
//         ],
//         annotations: {
//           yaxis: [
//             {
//               y: 4.5, // Set the threshold for the line
//               borderColor: '#FF0000',
//               label: {
//                 borderColor: '#FF0000',
//                 style: {
...
async function reports(req, res, next) {
  try {
    ...
    const chartData1 = {
      ...
      options: {
        ...
        annotations: {
          yaxis: [
            {
              y: 8, // Set the threshold for the line
              borderColor: "#FF0000",
              borderWidth: 5,
              label: {
                borderColor: "#FF0000",
            ...
            {
              y: 2, // Set the threshold for the line
              borderColor: "#FF0000",
              borderWidth: 5,
              label: {
                borderColor: "#FF0000",
    ...
    // Prepare the data for the second chart (values as the subtraction of the previous value)
    // Prepare the data for the second chart (values as the subtraction of the previous value)
    const chartData2 = {
      series: [
        {
    ...
    res.json({ chartData1, chartData2 });
  } catch (error) {
    next(error);
  }
}
